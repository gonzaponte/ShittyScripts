...
def print_folder(folder="./", prefix="", skip=(), ignore_hidden=True, output_stream=sys.stdout):
    for name in sorted(os.listdir(folder)):
        ...
        if os.path.isfile(fullname):
            ...
            print(prefix + name + " --->", file=output_stream)
            print_folder(fullname + "/", prefix + "\t", skip, ignore_hidden, output_stream)
        else:
...
print_folder(flags.f, skip=flags.s, ignore_hidden=not flags.show_hidden, output_stream=sys.stdout if flags.l is None else open(flags.l, "w"))
